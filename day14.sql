...
-- 하지만 특정 행 삭제시 PK가 아닌 조건으로 삭제하는 것은 
-- 주의해야 함.
...
--- 3. DELETE 의 WHERE 에 서브쿼리 조함 
-- (1) 주소가 '아산시'인 멤버인 사람을 모두 삭제 
...
--하나의 트랜젝션으로 4개의 DML 쿼리가 묶여 있는 상황 
--아직 COMMIT 되지 않았으므로 트랜잭션의 정상종료가 아닌 상황 

--------------------------------------------------------------
-- 홍길동의 데이터 ROLLBACK 시나리오 

-- 1. 주소 수정까지는 맞는데, 전화번호, 성별을 잘못 수정했다고 착각 ---사실은 성별만 잘못 수정 
--- : 되돌아갈 SAVEPOINT = do_update_addr
--- 사실은 성별만 잘못 수정
ROLLBACK TO do_update_addr; --롤백 세이브포인트를 잘못 잡음 
--롤백 완료.

-- 2. 주소 전화번호 수정까지는 맞았고, 성별 수정만 잘못되었음
ROLLBACK TO do_update_phone ;
--ORA-01086: 'DO_UPDATE_PHONE' 저장점이 이 세션에 설정되지 않았거나 부적합합니다.
--SVAPOONT 에는 순서가 있기 때문에 
--do_update_addr 이 do_update_phone 보다 앞서 생성되 지점이기 때문에 
--그후 생성된 SAVEPOINT 는 모두 삭제됨 


--3. 2번의 ROLLLBACK TO를 수행 후에 되돌릴 수 있는 지점 
...
-- 1. 시퀀스 생성 1. 시작번호 : 1, 최대 : 30, 사이클이 없는 시퀀스 생성 
...
--생성한  SEQ_MEMBER_ID 의 MAXVALUE 설정을 NOMAXVALUE로 변경 
...
-- SEQ_MEMBER_ID를 INCREMENT를 10으로 변경하려면 
ALTER SEQUENCE SEQ_MEMBER_ID
INCREMENT BY 10 
;
--Sequence SEQ_MEMBER_ID이(가) 변경되었습니다.
...
--new_member 의 member_id를 M001, M002 ...증가하는 형태로 조합 
...
-- 1. user_indexes 테이블에서 이미 존재하는 INDEX조회 
SELECT i.index_name
     , i.index_type
     , i.table_name
     , i.include_column
...
==> PK_MEMBER 라는 이름과 다른 IDX_MEMBER_ID 로 생성 시도해도 
    같은 컬럼에 대해서는 인덱스가 두 개 생성되지 않는다. 
*/

-- 3. 복사 테이블 new_member 에는 PK가 없기 떄문에 인덱스도 없는 상태 
