...
--           물리적 위치이므로 한 행당 반드시 유일한 값이 수 밖에 없음 
...
SELECT e.empno
 ...
 WHERE e.ename = 'SMITH'
;
--ROWID 같이 조회/물리적으로 오라클이 해석할 수 있는 위치에 스미스의 데이터가 들어있다
SELECT ROWID 
     , e.empno
     , e.ename
  FROM emp e
...
--     SUB-QUERY 를 썼을 때 ROWNUM 의 값 확인 
SELECT rownum
     , a.empno
     , a.ename
     , a.numrow 
...
SELECT e.empno
 ...
 ORDER BY e.sal DESC
;
--2. 1의 결과를 SUB-QUERY로 FROM 절 사용하여 
--   ROWNUM 을 같이 조회 
SELECT ROWNUM
     , a.*
  FROM (SELECT e.empno
...
--위처럼 VALUES절에 나열된 값의 개수가 전체 컬럼 수와 일치하지 않으면 
...
INSERT INTO MEMBER
VALUES ('M006', '홍길동', '0001', sysdate, '율도국', '도술', 0, 'M');
--ORA-02290: 체크 제약조건(SCOTT.CK_BIRTH)이 위배되었습니다
INSERT INTO MEMBER
...
INSERT INTO MEMBER
VALUES ('M006', NULL, '0001', sysdate, '율도국', '도술', -1, 'M');
--ORA-01400: NULL을 ("SCOTT"."MEMBER"."MEMBER_NAME") 안에 삽입할 수 없습니다
--NULL은 굳이 constraint 에 입력하지 않아도 됨 
...
*/
 ...
 UPDATE member m
    SET m.major = '역문컨'
--  WHERE member_id = 'M009' -- 실수로 WHERE 조건 누락 
;
/*
9개 행 이(가) 업데이트되었습니다.
...
-- 예) '안양시'에 사는 '송지환' 멤버의 'GENDER' 값 수정 
...
UPDATE member m
 ...
 -- 만약 SET 절에 사용하는 서브커리 결과가 
 -- 정확하게 1행 1열의 1개의 값이 아닌 경우 구문 오류 
 UPDATE member m
   SET m.major = (SELECT m.major 
                    FROM member m
                 )
 WHERE m.member_id = 'M002'
 ;
 --ORA-01427: 단일 행 하위 질의(서브쿼리)에 2개 이상의 행이 리턴되었습니다.
...
-- 예) M001 의 emeber_id 수정을 시도하는 경우 
--    : PK 컬럼 값을 중복 값으로 수정을 시도하는 경우
ROLLBACK;
UPDATE member m
