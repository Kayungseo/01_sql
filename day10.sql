...
COMMIT
*/
-- 6) OUTER JOIN : 조인 대상 테이블에서 공통 컬럼에 NULL값인 데이터도 
--                 출력을 원할 때 사용하는 조인 기법 
--      연산자 :(+), LEFT OUTER JOIN, RIGHT OUTER JOIN 

----1.(+)연산자 : 오라클에서만 사용하는 OUTER JOIN 연산자
--               EQUI-JOIN 조건에서 NULL값 출력을 원하는 쪽에 붙여서 사용 

--직원의 정보를 부서명과 함께 조회(일반적인 EQUI-JOIN)
SELECT e.empno
     , e.ename
     , d.dname
  FROM emp e
 ...
 WHERE e.deptno = d.deptno --오라클 전통적인 조인 조건 작성 방법 
;
--JJ, J_JAMES 의 e.deptno는 NULL이므로 dept테이블에 일치하는 값이 없으므로 
--조인결과로 두 직원이 조회되지 않는다. 

--부서 배치가 되지 않은 직원도 모두 출력은 하고 싶다. 
--그러려면 dept테이블쪽의 데이터가 NULL이어도 추가 출력이 필요.
...
SELECT e.empno
  ...
  FROM emp e
     , dept d
 WHERE e.deptno = d.deptno (+) --기준 table은 왼쪽, 따라서 더 출력하길 바라는 건 오른쪽 
;
/*
...
8888	J_JAMES	NULL
*/
--(+) 연산자는 오른쪽에 붙이는 NULL상태로 출력될 테이블을 결정짓는다. 
...
--RIGHT OUTER JOIN ~ON으로 변경(same as upside)
SELECT e.empno "사번"
   , e.ename "이름"
   , e.mgr   "상사 사번"
   , e1.ename"상사 이름"
